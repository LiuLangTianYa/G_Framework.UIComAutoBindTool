...
public class AutoBindGlobalSetting : ScriptableObject
{
    ...
    private List<AutoBindRulePrefixe> m_RulePrefixes= new List<AutoBindRulePrefixe>()
    {
        ...
        new AutoBindRulePrefixe("Anim","Animator"),
        ...
        new AutoBindRulePrefixe("Btn","Button"),
        new AutoBindRulePrefixe("Img","Image"),
        new AutoBindRulePrefixe("RImg","RawImage"),
        new AutoBindRulePrefixe("TextM","TMP_Text"),
        new AutoBindRulePrefixe("Text","Text"),
        new AutoBindRulePrefixe("InputM","TMP_InputField"),
        new AutoBindRulePrefixe("Input","InputField"),
        ...
        new AutoBindRulePrefixe("Drop","TMP_Dropdown"),
    ...
    public static bool IsValidBind( Transform target, List<string> filedNames, List<string> componentTypeNames)
    {
        ...
        for (int i = 0; i < strArray.Length - 1; i++)
        {
            ...
            if (!isFindComponent)
            {
                Log.Error($"{target.name}的命名中{str}不存在对应的组件类型，绑定失败");
            }
        }
        if (!isFind)
        {
    ...
    public static AutoBindGlobalSetting GetAutoBindGlobalSetting()
    {
        ...
        if (paths.Length == 0)
        {
            Log.Error("不存在AutoBindGlobalSetting");
            return _AutoBindGlobalSetting;
        }
        if (paths.Length > 1)
        {
            Log.Error("AutoBindGlobalSetting数量大于1");
            return _AutoBindGlobalSetting;
        }
        string path = AssetDatabase.GUIDToAssetPath(paths[0]);
